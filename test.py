...
mag, tempir, uvlight, magx, magy, magz,dist,orientx,orienty = [], [], [], [], [], [],[],[],[]
...
def lineGraph(xlabel,ylabel,title,X,y,xlimi,xlimf,ylimi,ylimf):
    plt.scatter(X,y,color='red')
    plt.plot(X,y,color = 'blue')
    plt.gca().set_ylim([ylimi,ylimf])
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.show()
...
class FolderMonitor(FileSystemEventHandler):
    def on_modified(self, event):
        ...
        orientx.append(dataset.iloc[-1:, 5:6].values.tolist()[0][0])
        orienty.append(dataset.iloc[-1:, 6:7].values.tolist()[0][0])
        for x, y, z in zip(magx, magy, magz):
            mag.append(sqrt(x * x + y * y + z * z))
        for x, y in zip(orientx,orienty):
            orientation = sqrt(x*x+y*y)
            dist.append((orientation/360)*2*pi*(MARS_RADIUS+DISTANCE_FROM_SURFACE))
            print(dist)
        del mag[len(dataset):]
